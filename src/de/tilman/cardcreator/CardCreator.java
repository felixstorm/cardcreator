	...
	public CardCreator(String cardsFile) {
		...
		try {
			...
			for (int i = links.size() - 1; i >= 0; i--)
				...
				else if (links.get(i).startsWith("spotify:") && links.get(i).indexOf(' ') > 0)
					links.set(i, links.get(i).substring(0, links.get(i).indexOf(' ')));
			log.info("Read " + links.size() + " links from " + cardsFile);

			if (properties.getProperty("spotifyClientId") != null
					&& properties.getProperty("spotifyClientId").length() > 0) {
				spotifyApi = new SpotifyApi.Builder().setClientId(properties.getProperty("spotifyClientId"))
						.setClientSecret(properties.getProperty("spotifyClientSecret")).build();
		...
		for (String link : links) {
			...
			if (link.startsWith("spotify:album:")) {
				...
				if (link.startsWith("http://") || link.startsWith("https://")) {
					...
					if (card.cover == null) {
						String path = properties.getProperty("sharePath") + link.substring(link.indexOf("/", 4))
								+ "/cover.jpg";
						log.debug(path);
